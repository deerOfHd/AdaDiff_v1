...
args.save = args.save + '-niter-{}'.format(args.epochs) + '-optimizer-{}'.format(args.optimizer) + '-nlayers{}'.format(
    args.nlayers) + \
            '-lr{}'.format(args.lr) + '-clip-{}'.format(args.clip) + '-eps{}'.format(args.eps) \
            + '-epsqrt{}'.format(args.eps_sqrt) + '-betas-{}-{}'.format(args.beta1, args.beta2) + '-run{}'.format(
    args.run) + '-wdecay{}-when-{}'.format(args.wdecay, args.when)

args.tied = True

if not os.path.isdir(os.path.join('curve', 'nlayer-' + str(args.nlayers))):
    os.makedirs(os.path.join('curve', 'nlayer-' + str(args.nlayers)))
...
try:
    ...
    if args.optimizer == 'ada1':
        optimizer = Ada1(params, lr=args.lr, weight_decay=args.wdecay)
    if args.optimizer == 'ada2':
        optimizer = Ada2(params, lr=args.lr, weight_decay=args.wdecay,
                         eps=args.eps, betas=(args.beta1, args.beta2))
    if args.optimizer == 'ada3':
        optimizer = Ada3(params, lr=args.lr, weight_decay=args.wdecay,
                         eps=args.eps, betas=(args.beta1, args.beta2))
    if args.optimizer == 'ada4':
        optimizer = Ada4(params, lr=args.lr, weight_decay=args.wdecay,
                         eps=args.eps, betas=(args.beta1, args.beta2))
    if args.optimizer == 'eadam':
        optimizer = EAdam(params, lr=args.lr, weight_decay=args.wdecay, eps=args.eps)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            if val_loss2 < stored_loss:
                model_save(os.path.join('save', args.save))
        ...
        if epoch in args.when:
            ...
            model_save(os.path.join('save', '{}.e{}'.format(args.save, epoch)))
        ...
        torch.save({'train_loss': train_losses, 'test_loss': val_losses},
                   os.path.join('curve', 'nlayer-' + str(args.nlayers), args.save))
...
model_load(os.path.join('save', args.save))
...
print('=' * 89)
if os.path.exists(os.path.join('save', args.save)):
  os.remove(os.path.join('save', args.save))
else:
